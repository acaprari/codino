  ...
  return (
    <AuroraModal open={open} onClose={() => {}} maxWidth={640}>
      <div style={{ textAlign: 'center', fontFamily: 'var(--aurora-font-ui)' }}>
        ...
        <div style={{ fontSize: '24px', letterSpacing: '4px', color: 'var(--aurora-accent-amber)', textShadow: '0 0 14px rgba(var(--aurora-accent-amber-rgb), 0.6)', marginBottom: '10px' }}>
        ...
        <div style={{ background: 'rgba(var(--aurora-bg-glow-violet-rgb), 0.10)', borderLeft: '3px solid var(--aurora-accent-purple)', padding: '12px 16px', borderRadius: 'var(--aurora-card-radius)', textAlign: 'left', marginBottom: '20px', color: 'var(--aurora-text-primary)', lineHeight: 1.5 }}>
          {narrativeBridge}
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '10px', flexWrap: 'wrap' }}>
          {branches.map((b, i) => (
            <button
              ...
              onClick={() => onPick(b)}
              style={{
                background: 'linear-gradient(135deg, rgba(var(--aurora-accent-purple-rgb), 0.30), rgba(var(--aurora-accent-pink-rgb), 0.30))',
                border: '1px solid rgba(var(--aurora-bg-glow-violet-rgb), 0.50)',
                borderRadius: '12px',
                padding: '14px 18px',
                cursor: 'pointer',
                display: 'flex',
